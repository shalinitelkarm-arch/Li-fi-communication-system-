...
void loop() {
  ...
  if (currentState != lastState) {
    ...
    // *** FIX 2: Check for LIGHT pulse (state 0) ***
    ...
    if (lastState == 0) { 
      ...
      // If it was too short/long, ignore it (noise)
    } 
  ...
  // *** FIX 3: Check for DARK state (state 1) ***
  // --- 2. Check for "End of Transmission" ---
  // If it's dark AND it's been dark for a long time...
  if (currentState == 1 && (now - lastChangeTime > end_of_transmission_gap)) {
    ...
    if (currentMessage.length() > 0) {
      ...
      if (currentMessage.length() > 16) {
        lcd.print(currentMessage.substring(0, 16)); // Print first 16 chars
        delay(2000);
      ...
      currentMessage = "";
      delay(5000); // Show message for 3 seconds
      lcd.clear();
      lcd.print("Ready...");
    }
